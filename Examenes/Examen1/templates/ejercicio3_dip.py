...
Sistema de Biblioteca con inyección de dependencias
"""
...
class IRepositorio(ABC):
    """Interfaz abstracta para repositorios (DIP)"""
...
class RepositorioArchivo(IRepositorio):
    """Implementación: Persistencia en archivo"""

    def __init__(self, archivo="biblioteca_dip.txt"):
    ...
    def guardar(self, libros, prestamos):
        try:
            ...
            print(f"[REPOSITORIO ARCHIVO] Datos guardados en '{self.archivo}'")
            return True
        except Exception as e:
            print(f"[REPOSITORIO ARCHIVO] Error al guardar: {e}")
            return False
    ...
    def cargar(self):
        try:
            with open(self.archivo, "r", encoding="utf-8") as f:
                data = f.read()
            print(f"[REPOSITORIO ARCHIVO] Datos cargados desde '{self.archivo}'")
            return True, data
        except FileNotFoundError:
            print(f"[REPOSITORIO ARCHIVO] Archivo '{self.archivo}' no encontrado")
            return False, "Archivo no encontrado"
        except Exception as e:
            print(f"[REPOSITORIO ARCHIVO] Error al cargar: {e}")
            return False, f"Error al cargar: {e}"


class RepositorioMemoria(IRepositorio):
    """Implementación BONUS: Persistencia en memoria"""
    ...
    def guardar(self, libros, prestamos):
        try:
            ...
            print("[REPOSITORIO MEMORIA] Datos guardados en memoria")
            return True
        except Exception as e:
            print(f"[REPOSITORIO MEMORIA] Error al guardar: {e}")
            return False

    def cargar(self):
        try:
            if self.datos_guardados is None:
                print("[REPOSITORIO MEMORIA] No hay datos en memoria")
                return False, "No hay datos en memoria"

            data = f"Libros: {len(self.datos_guardados['libros'])}\n"
            data += f"Préstamos: {len(self.datos_guardados['prestamos'])}\n"

            print("[REPOSITORIO MEMORIA] Datos cargados desde memoria")
            return True, data
        except Exception as e:
            print(f"[REPOSITORIO MEMORIA] Error al cargar: {e}")
            return False, f"Error al cargar: {e}"


class SistemaBiblioteca:
    """Sistema de biblioteca con Dependency Inversion Principle"""

    def __init__(self, validador, repositorio: IRepositorio, notificador):
        self.libros = []
        self.prestamos = []
...
def main():
    print("=" * 60)
    print("EJERCICIO 3: Dependency Inversion Principle (DIP)")
    print("=" * 60)

    validador = ValidadorBiblioteca()
    notificador = ServicioNotificaciones()

    print("\n--- Configurando con RepositorioArchivo ---")
    ...
    print("\n" + "=" * 60)
    print("BONUS: Cambiando a RepositorioMemoria")
    print("=" * 60)

    repositorio_memoria = RepositorioMemoria()
    sistema2 = SistemaBiblioteca(validador, repositorio_memoria, notificador)

    print("\n=== AGREGANDO LIBROS CON REPOSITORIO EN MEMORIA ===")
    print(sistema2.agregar_libro("Don Quijote", "Miguel de Cervantes", "9788424936464"))
